...
def load_samples(f_shp, reproject):
	...
	_schema = _lyr.schema
	_ns = []
	for i in xrange(len(_schema)):
		_ns.append(_schema[i].name)
	del _schema
	...
	for _f in _lyr:
		_pt = _f.geometry()
		_pp = _pt.GetPoint_2D()
		_p1 = float(_pp[0]), float(_pp[1])
		del _pp

		_vs.append([_f.items()[_n] for _n in _ns])

		_prj = None
		if reproject:
			_prj = _pt.GetSpatialReference()
			# _prj = _prj.Clone()

		if not (_v_min <= _p1[0] <= _v_max and _v_min <= _p1[1] <= _v_max):
			print ' * skip invalid loc: %s %s' % tuple(_p1)
			continue

		_ss.append(geo_raster_ex_c.geo_point(_p1[0], _p1[1], _prj))
